...
3. Run:
   - `sudo bash scripts/deploy-Project.sh`
...
- `sudo SYNC_REPO=true bash scripts/deploy-Project.sh`

By default it stashes local changes, syncs to `origin/main`, then deploys.
If you want to discard local changes instead:
- `sudo SYNC_REPO=true FORCE_RESET=true bash scripts/deploy-Project.sh`

### Helpful flags
- Auto-generate required secrets (replaces placeholder values in `.env.production`):
   - `sudo AUTO_GENERATE_SECRETS=true bash scripts/deploy-Project.sh`
- Start with self-signed (dummy) certs and skip Let's Encrypt issuance (useful before DNS points to the VPS):
   - `sudo SKIP_LETSENCRYPT=true bash scripts/deploy-Project.sh`

Optional:
- Start monitoring too: `sudo START_MONITORING=true bash scripts/deploy-Project.sh`
...
- `sudo bash scripts/Project.sh restart`
