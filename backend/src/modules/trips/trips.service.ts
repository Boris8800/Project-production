...
export class TripsService {
  ...
  async assignDriver(adminUserId: string, bookingId: string, driverId: string) {
    ...
    if (existingTrip) {
      if (existingTrip.status === TripStatus.Completed || existingTrip.status === TripStatus.Cancelled) {
        throw new BadRequestException('Trip not assignable');
      }
  ...
  async acceptTrip(driverId: string, tripId: string) {
    ...
    if (trip.status !== TripStatus.Pending) {
      throw new BadRequestException(`Trip not accept-able from status: ${trip.status}`);
    }
  ...
  async startTrip(driverId: string, tripId: string) {
    ...
    if (trip.status === TripStatus.Completed || trip.status === TripStatus.Cancelled) {
      throw new BadRequestException(`Trip not start-able from status: ${trip.status}`);
    }

    // Allow idempotent start.
  ...
  async completeTrip(driverId: string, tripId: string, distanceM?: number, durationS?: number) {
    ...
    if (trip.status !== TripStatus.Started) {
      throw new BadRequestException(`Trip not complete-able from status: ${trip.status}`);
    }
  ...
  async getTripForUser(userId: string, role: Role | undefined, tripId: string) {
    const trip = await this.trips.findOne({ where: { id: tripId } });
    if (!trip) throw new NotFoundException('Trip not found');

    const isAdmin = role === Role.Admin || role === Role.SuperAdmin;
    const isDriverOwner = role === Role.Driver && trip.driverId === userId;
    if (!isAdmin && !isDriverOwner) throw new ForbiddenException();
