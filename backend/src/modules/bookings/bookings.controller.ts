...
export class BookingsController {
  ...
  @Get(':id')
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.Customer, Role.Driver, Role.Admin, Role.SuperAdmin)
  async get(@Req() req: Request & { user?: unknown }, @Param('id') id: string) {
    const user = req.user as { id: string; role?: Role };
    return this.bookings.getBookingForUser(user.id, user.role, id);
  }
}
